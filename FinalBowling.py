...
import time
import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QLabel, QPushButton, QVBoxLayout, QWidget, QSlider, QFileDialog, QHBoxLayout
...
class PoseComparisonApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.select_videos_button = QPushButton('Select Videos', self)
        self.select_videos_button.clicked.connect(self.select_videos)

        self.clear_graph_button = QPushButton('Clear Graph', self)
        ...
        self.buttons_layout.addWidget(self.clear_graph_button)
        self.layout.addLayout(self.buttons_layout)  # Add buttons layout to the main layout
    ...
    def select_videos(self):
        ...
        benchmark_video, _ = QFileDialog.getOpenFileName(self, "Select Benchmark Video", "", "Video Files (*.mp4 *.avi);;All Files (*)", options=options)
        user_video, _ = QFileDialog.getOpenFileName(self, "Select User Comparison Video", "", "Video Files (*.mp4 *.avi);;All Files (*)", options=options)

        if benchmark_video and user_video:
            self.benchmark_cam = cv2.VideoCapture(benchmark_video)
            self.user_cam = cv2.VideoCapture(user_video)

    def update_frame(self):
        if not self.paused and self.benchmark_cam is not None and self.user_cam is not None:
            try:
                ...
                self.ax_accuracy.set_ylim(0, 150)  # Set the y-axis limit for accuracy
    ...
    def clear_graph(self):
        self.errors_user = []
        self.errors_benchmark = []
        self.accuracy = []
        self.correct_frames = 0
        self.frame_counter = 0
        self.ax.cla()  # Clear the current axes in the Matplotlib plot
        self.ax_difference.cla()  # Clear the current axes for error difference
        self.ax.set_title("User and Benchmark Error")
        self.ax.set_xlabel("Frame")
        self.ax.set_ylabel("Error")
        
        # Reset y-axis limit for accuracy in the first subplot
        self.ax_accuracy.clear()  # Clear the accuracy axis
        self.ax_accuracy = self.ax.twinx()  # Use twinx for the same y-axis
        self.accuracy_line, = self.ax_accuracy.plot([], [], label='Accuracy', color='brown')
        self.ax_accuracy.set_ylim(0, 150)  # Set the y-axis limit for accuracy
        ...
        # Reset y-axis limit for accuracy in the second subplot
        self.ax_accuracy.clear()  # Clear the accuracy axis
        self.ax_accuracy = self.ax_difference.twinx()  # Use twinx for the same y-axis
        self.accuracy_line, = self.ax_accuracy.plot([], [], label='Accuracy', color='brown')
        self.ax_accuracy.set_ylim(0, 150)  # Set the y-axis limit for accuracy
        ...
        self.ax.legend(loc='upper left')
        self.ax_accuracy.legend(loc='upper right')
        self.canvas.draw()


    def closeEvent(self, event):
        if self.benchmark_cam:
